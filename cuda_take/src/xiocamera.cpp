...
void XIOCamera::setDir(const char *dirname)
{
    ...
    */
    ...
    cout << "Finished for loop in dir file search (part of setDir)";
    running.store(true);
    //emit started(); // seems to work without this

    is_reading = true;
    //readLoopFuture = QtConcurrent::run(this, &XIOCamera::readLoop);
    cout << "Finished XIO setDir.";
}

std::string XIOCamera::getFname()
{
    cout << __PRETTY_FUNCTION__ << "Starting getFname() in XIO camera";
...
void XIOCamera::readFile()
{
    ...
    while(!validFile) {
        ...
        if (ifname.empty()) {
            ...
            cout << __PRETTY_FUNCTION__ << "All out of files, give up.";
        ...
        if (!dev_p.is_open()) {
            cout << "Could not open file" << ifname.data() << ". Does it exist?";
            dev_p.clear();
            readFile();
            return;
        }
        ...
        if (!std::strcmp(ext.data(), "decomp")) {
            ...
            cout << __PRETTY_FUNCTION__ << "decomp read, filesize read from actual file stream is: " << filesize;
        } else {
            ...
            cout << __PRETTY_FUNCTION__ << "Not-decomp, filesize read from header is: " << filesize;
        }
        ...
        if (framesize == 0) { //If header reports a 0 filesize (invalid data), then skip this file.
            ...
            validFile = true;
            cout <<__PRETTY_FUNCTION__<< "rsv - dev_p.seekg()" << headsize  ;
            dev_p.seekg(headsize, std::ios::beg);

            cout <<__PRETTY_FUNCTION__<< "File size is" << filesize << "bytes, which corresponds to a framesize of" << framesize << "bytes.";
            ...
            for (volatile int n = 0; n < nFrames; ++n) {
                ...
                cout << __PRETTY_FUNCTION__ << "Read " << read_size << " bytes from frame " << n;
                ...
                if (framesize / sizeof(uint16_t) < size_t(frame_width * data_height)) {
                    ...
                    cout << __PRETTY_FUNCTION__ << "size of frame_buf:" << frame_buf.size();
                    //qDebug() << __PRETTY_FUNCTION__ << "size of frame_buf[0]:" << frame_buf[0].size();
                    cout << __PRETTY_FUNCTION__ << "framesize:" << framesize << ",framesize in uint16 size:     " << framesize/sizeof(uint16_t);
                    cout << __PRETTY_FUNCTION__ << "frame_width: " << frame_width << ", data_height: " << data_height << ", product: " << frame_width*data_height;
            ...
            running.store(true);
            cout << __PRETTY_FUNCTION__ << "About done, emitting started signal.";
            //emit started(); // doesn't seem to be needed?
            dev_p.close();
        }
    }
    cout << __PRETTY_FUNCTION__ << " is done.";
}

void XIOCamera::readLoop()
{
    cout << __PRETTY_FUNCTION__ << "Entering readLoop()";
    int waits = 1;
    do {
        ...
        if (frame_buf.size() <= 96) {
            ...
            cout << __PRETTY_FUNCTION__ << "Waiting: Wait step: " << waits++; // hapens 8 times in between files.
