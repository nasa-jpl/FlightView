...
void acquire::loadDSFMask_entry(std::string filename_s, fileFormat_t fmt) {
    ...
    case fmt_float32:
        ...
        return;
        mask_thread = boost::thread(&acquire::loadDSFMask, this, filename_s);
        break;
    default:
...
void acquire::loadDSFMask(std::string file_name)
{
    if(readingDSFFile)
        return;
...
void acquire::savingLoop(std::string fname, unsigned int num_avgs, unsigned int num_frames) 
{
