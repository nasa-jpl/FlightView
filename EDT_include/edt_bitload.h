...
EDTAPI int edt_program_mezzanine(EdtDev *edt_p, const u_char *buf, int size, int channel);
EDTAPI void edt_bitfile_destroy(EdtBitfile *bfd);

#ifdef DO_DIRECT_LOAD
EDTAPI int edt_program_xilinx_direct(EdtDev *edt_p, const u_char *buf, int size, int do_sleep);
