 ...
 *
 * @note When developing applications for EDT Digital Video boards such as
 * the PCIe8 DV C-Link, programmers should use the higher level \ref dv. Some
 * limited use of edtlib calls may be necessary in DV applications, however
 * we can not provide support for applications that use edtlib calls for data
 * acquisition (e.g. edt_configure_ring_buffers, edt_start_buffers); instead use
 * the pdvlib corralaries (e.g. pdv_multibuf, pdv_start_images).
 *
 ...
 * and link the library file libedt.c.
 *
 * Assuming that a multichannel Xilinx firmware file has been loaded,
...
#define EDTAPI_VERSION 0x05030202
...
EDTAPI void edt_print_flashstatus(char stat, int sector, int frdata);
...
EDTAPI void edt_program_promdata(EdtDev *edt_p, int promcode, int sector, EdtPromData *pdata);
...
EDTAPI int    edt_x_prom_detect(EdtDev *edt_p, u_char *stat);
...
EDTAPI char * edt_x_type_string(int xtype);
...
EDTAPI int edt_program_xilinx_start(EdtDev *edt_p);
EDTAPI void edt_program_xilinx_chunk(EdtDev *edt_p, const u_char *buf, int xfer, int do_sleep);
EDTAPI int edt_program_xilinx_end(EdtDev *edt_p);
EDTAPI int edt_get_x_file_header(const char *fname, char *header, int *size);
...
EDTAPI int edt_x_get_fname(EdtDev *edt_p, char *name);
EDTAPI int edt_x_get_fname_auto(EdtDev *edt_p, char *name);
...
#define EDT_N_BDTYPES	6 /* really 5, but include UNKN */
...
#define ID_IS_FCIUSPS(id) (   (id == PDVFCI_USPS_ID) \
			   || (id == PC104ICB_ID))

#define ID_HAS_IRIGB(id) (  (id == PE8DVCL_ID) \
			 || (id == PE4DVCL_ID) \
			 || (id == PE4DVAFOX_ID))

/* ADD any devices that don't have a separate interface FPGA here */
#define ID_HAS_COMBINED_FPGA(id) ( (id == P11W_ID) \
				|| (id == P16D_ID) \
				|| (id == PDVCL_ID) \
				|| (id == PE4DVCL_ID) \
				|| (id == PE8DVCL_ID) \
